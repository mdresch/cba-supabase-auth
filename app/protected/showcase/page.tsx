...
const ProductSkeleton = dynamic(() => import("@/components/product-skeleton"), {
  loading: () => <div className="skeleton" />,
});

// Define the Product type (interface or type alias)
interface Product {
  id: number;
  name: string;
  category: string;
...
export default function Showcase() {
    ...
    const [displayedProducts, setDisplayedProducts] = useState<Product[]>([]); // Use the Product type here
    const [skeletonCount, setSkeletonCount] = useState(3);
    const [page, setPage] = useState(1);

    

    const categories = useMemo(() => ["All", ...new Set(allProducts.map((product) => product.category))], []);

    const loadProducts = async (): Promise<void> => {  // Add the return type Promise<void>
      setLoading(true);
      setDisplayedProducts([]); // This should now be fine
    
      const filteredProducts: Product[] = filter === "All" // Explicitly type filteredProducts
          ? allProducts
          : allProducts.filter((product: Product) => product.category === filter); // Type the product parameter in the filter
    
      const productsToLoad: Product[] = filteredProducts.slice(0, ITEMS_PER_PAGE); // Type productsToLoad
    
      setDisplayedProducts(productsToLoad); // Now this should work!
      setLoading(false);
    };
            ...
            >
                {categories.map((category, index) => (
                    <motion.div
                        ...
                        whileTap={{ scale: 0.95 }}
                    >
                        <Button onClick={() => setFilter(category)} variant={filter === category ? "default" : "outline"} aria-pressed={filter === category}>
                            {category}
                        </Button>
                    </motion.div>
                ))}
            ...
            >
                <AnimatePresence mode="popLayout">
                    {loading && displayedProducts.length === 0 && (
                        [...Array(skeletonCount)].map((_, index) => ( // Removed Fragment here
                    ...
                    {displayedProducts.map((product, index) => (
                        ...
                        >
                            <Image
                                ...
                                priority={index < ITEMS_PER_PAGE} // Correct: priority for the first few
                                loading={index >= ITEMS_PER_PAGE ? "lazy" : undefined} // Conditional lazy load
                            />
